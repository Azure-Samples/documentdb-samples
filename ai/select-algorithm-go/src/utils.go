...
func LoadConfig() *Config {
	...
	return &Config{
		...
		VectorField:  getEnvOrDefault("EMBEDDED_FIELD", "contentVector"),
		ModelName:    getEnvOrDefault("AZURE_OPENAI_EMBEDDING_MODEL", "text-embedding-3-small"),
		Dimensions:   dimensions,
		BatchSize:    batchSize,
		Similarity:   getEnvOrDefault("SIMILARITY", "COS"),
		Algorithm:    strings.ToLower(getEnvOrDefault("ALGORITHM", "all")),
	}
}
