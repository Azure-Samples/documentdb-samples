...
export function getConfig() {
    ...
    const embeddedField = process.env.EMBEDDED_FIELD!;
    const embeddingDimensions = parseInt(process.env.EMBEDDING_DIMENSIONS!, 10);
    const deployment = process.env.AZURE_OPENAI_EMBEDDING_MODEL!;
    const similarity = process.env.SIMILARITY || 'COS';

    return { dbName, dataFile, batchSize, embeddedField, embeddingDimensions, deployment, similarity };
}
